...
function render(iStart, jStart){
	...
	var rO = Math.floor(Math.random() * 256) - r;
	var bO = Math.floor(Math.random() * 256) - b;
	var gO = Math.floor(Math.random() * 256) - g;
	
	for(var i = 0; i < width; i++){
		for(var j = 0; j < height; j++){
			
			var percent = cost[i + (j * width)] / max;
			
			var rC = Math.floor(percent * rO) + r;
			var bC = Math.floor(percent * bO) + b;
			var gC = Math.floor(percent * gO) + g;
			ctx.fillStyle = "rgb(" + rC + ", " + bC + ", " + gC + ")";
